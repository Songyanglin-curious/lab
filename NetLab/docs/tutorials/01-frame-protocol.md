...
- [X] 帧头 EB 90 被篡改了，接收方会怎样？
  会报错，报帧头错误
- [X] 为什么长度字段要逐字节读，不能一次性 Read(4)？
  因为变长代表着表示长度不固定，一字节一字节的才知道这个byte究竟是代表长度还是数据，如果数据长度只有1字节那么一次读4字节就会1部分是表示长度一部分是表示数据，处理起来逻辑就相当费劲了
- [X] 如果客户端发 3 条消息，服务端现在收到几条？为什么？
  如果从TCP的角度看那就是不知道，如果从解析的结果看那就是3条，因为现在有帧协议限制了
